...
class Cube(context: Context, res: Resources) : View(context) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (side in verticesToSide[bottomPoint]!!){
            ...
            path.moveTo(center[0]+vertices[side[0]][0]*coef, center[1]+vertices[side[0]][1]*coef)
            for (ind in side){
                path.lineTo(center[0]+vertices[ind][0]*coef, center[1]+vertices[ind][1]*coef)
                canvas.drawPoint(center[0]+vertices[ind][0]*coef, center[1]+vertices[ind][1]*coef, verticesPaint)
            }
            path.close()
